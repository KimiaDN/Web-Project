<html dir="rtl">
    <head>
        <meta charset="utf-8"> 
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>کلینیک سلامت</title>
        <link rel="stylesheet" href="css/bootstrap.rtl.css">
	<link type="text/css" rel="stylesheet" href="poshtibani.css">
    </head>
    <body>
        <header class="sticky">
            <div class="container-fluid" id="pageHeader">
                <div class="row">
                    <div class="col-md-1 py-2">
                        <img src="img/logo.jpg" class="rounded" width="100" height="100">
                    </div>
                    <div class="col-md-8 py-5" id="pageTitle">
                        <p>پشتیبانی سامانه کلینیک سلامت</p>
                    </div>
                </div>
            </div>
            <nav>
                <ol class="breadcrumb px-3 bg-light">
                    <li class="breadcrumb-item"><a href="asli.php">صفحه اصلی</a></li>
                    <li class="breadcrumb-item active">پشتیبانی</li>
                </ol>
            </nav>
        </header>
        <div class="py-5">
            <div class="container">
                <div class="row d-flex justify-content-center">
                    <div class="col-md-8">
                        <h3>تلفن های پشتیبانی</h3>
                        <div class="d-flex justify-content-center">
                            <p class="phoneNumber d-flex justify-content-center align-items-center">013-32114567</p>
                        </div>
                        <p>همچنین در واتساپ، پاسخگوی شما هستیم</p>
                        <div class="d-flex justify-content-center">
                            <p class="phoneNumber d-flex justify-content-center align-items-center">09112345678</p>
                        </div>
                        <br>
                        <p>همکاران ما در اسرع وقت، پاسخگوی سوالات شما هستند.</p>
                        <p>بهترین زمان پاسخگویی : 9 صبح الی 9 شب</p>
                    </div>
                    <div class="col-md-2">
                    <img id="photo" src="img/poshtiban.png">
                    </div>
                </div>  
            </div>
        </div>
        <div class="container">
            <footer class="d-flex flex-wrap justify-content-between align-items-center py-3 my-3 border-top">
                <p class="col-md-4 mt-0 text-muted">
                    آدرس : رشت، گلسار، نبش کوچه 110
                    <br>
                    ساختمان سلامت
                </p>
                <div class="col-md-4 d-flex align-items-center justify-content-center mb-3 mb-md-0 me-md-auto text-decoration-none">
                    <img src="img/clinic.jpg" class="rounded" width="150">
                </div>
                <div class="col-md-4 mb-0 text-muted">
                    شماره های تماس :
                    <p class="d-flex justify-content-end">013-32117683</p>
                    <p class="d-flex justify-content-end">013-32117689</p>
                </p>
            </footer>
        </div>
	<script src="js/bootstrap.bundle.js"></script>
    </body>
</html>
